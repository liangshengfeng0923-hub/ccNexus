export default {
    common: {
        dashboard: '仪表板',
        endpoints: '端点',
        statistics: '统计',
        testing: '测试',
        apikeys: 'API 密钥',
        toggleTheme: '切换主题',
        toggleLanguage: '切换语言',
        add: '添加',
        edit: '编辑',
        delete: '删除',
        save: '保存',
        update: '更新',
        create: '创建',
        cancel: '取消',
        confirm: '确认',
        close: '关闭',
        loading: '加载中...',
        noData: '暂无数据',
        name: '名称',
        status: '状态',
        actions: '操作',
        enabled: '启用',
        disabled: '禁用',
        active: '活动',
        inactive: '非活动',
        success: '成功',
        failed: '失败',
        error: '错误',
        warning: '警告',
        info: '信息',
        total: '总计',
        refresh: '刷新',
        copy: '复制',
        clone: '克隆',
        test: '测试',
        switch: '切换',
        import: '导入',
        export: '导出',
        required: '必填',
        optional: '可选'
    },
    dashboard: {
        title: '仪表板',
        subtitle: 'AI 代理管理',
        totalRequests: '总请求数',
        successRate: '成功率',
        inputTokens: '输入令牌',
        outputTokens: '输出令牌',
        activeEndpoints: '活动端点',
        recentActivity: '最近活动',
        requests: '请求数',
        noEndpoints: '未配置端点',
        noEnabledEndpoints: '无启用端点'
    },
    endpoints: {
        title: '端点管理',
        addEndpoint: '添加端点',
        name: '名称',
        apiUrl: 'API 地址',
        apiKey: 'API 密钥',
        transformer: '转换器',
        model: '模型',
        tokenPool: '令牌池',
        status: '状态',
        actions: '操作',
        current: '当前',
        notTested: '未测试',
        testPassed: '测试通过',
        testFailed: '测试失败',
        switchToEndpoint: '切换到此端点',
        copyUrl: '复制 URL',
        tokenPoolManagement: '令牌池管理',
        deleteEndpoint: '删除端点',
        cloneEndpoint: '克隆端点',
        noEndpoints: '暂无端点',
        noEndpointsMessage: '添加您的第一个端点以开始使用',
        editEndpoint: '编辑端点',
        addEndpoint: '添加端点',
        cloneEndpoint: '克隆端点',
        remark: '备注',
        fetchModels: '获取模型',
        fetchModelsHint: '点击"获取模型"从 API 加载可用模型',
        modelPlaceholder: 'gpt-4、gemini-pro 等',
        apiKeyPlaceholder: 'sk-...',
        apiUrlPlaceholder: 'https://api.example.com',
        keepExistingKey: '保持为 **** 以保留现有密钥',
        selectModel: '选择模型',
        noModelsFound: '未找到模型',
        testEndpoint: '测试端点',
        testing: '测试中...',
        testSuccessful: '测试成功！延迟：',
        testFailed: '测试失败：',
        testResult: '测试结果',
        latency: '延迟',
        response: '响应',
        noResponse: '无响应',
        confirmDelete: '确定要删除端点"{name}"吗？',
        endpointCreated: '端点创建成功',
        endpointUpdated: '端点更新成功',
        endpointDeleted: '端点删除成功',
        endpointCloned: '端点已克隆。请保存以创建新端点。',
        endpointSwitched: '已切换到端点：',
        endpointEnabled: '端点已启用',
        endpointDisabled: '端点已禁用',
        endpointsReordered: '端点重新排序成功',
        failedToLoad: '加载失败',
        failedToSave: '保存失败',
        failedToDelete: '删除失败',
        failedToToggle: '切换失败',
        failedToSwitch: '切换失败',
        failedToReorder: '重新排序失败',
        failedToTest: '测试失败',
        failedToFetchModels: '获取模型失败',
        failedToClone: '克隆失败',
        failedToCopy: '复制到剪贴板失败',
        urlCopied: 'URL 已复制',
        enterApiUrlAndKey: '请先输入 API 地址和 API 密钥',
        modelSelected: '已选择模型：',
        tokenPoolTitle: '令牌池：',
        total: '总计',
        active: '活动',
        expiring: '即将过期',
        needRefresh: '需要刷新',
        expired: '已过期',
        invalid: '无效',
        cooldown: '冷却中',
        activate: '激活',
        batchImportJson: '批量导入 JSON',
        jsonPasteHint: '粘贴一项 / 数组 / {"items":[...]}',
        overwriteExisting: '覆盖现有的 account_id/email',
        import: '导入',
        id: 'ID',
        account: '账户',
        email: '邮箱',
        expiresAt: '过期时间',
        lastError: '最后错误',
        noCredentials: '未导入凭证',
        importDone: '导入完成：新增 {created}，更新 {updated}，跳过 {skipped}，失败 {failed}',
        credentialEnabled: '凭证已启用',
        credentialDisabled: '凭证已禁用',
        credentialActivated: '凭证已激活',
        tokenUpdated: '凭证令牌已更新',
        credentialDeleted: '凭证已删除',
        failedToLoadTokenPool: '加载令牌池失败',
        failedToImport: '导入失败',
        failedToUpdateCredential: '更新凭证失败',
        failedToActivateCredential: '激活凭证失败',
        failedToDeleteCredential: '删除凭证失败',
        enterAccessToken: '新访问令牌',
        enterExpiresAt: '过期时间（RFC3339，可选）',
        confirmDeleteCredential: '确定要删除凭证 #{id} 吗？',
        pleasePasteJson: '请先粘贴凭证 JSON',
        invalidJson: '无效的 JSON',
        failedToUpdateToken: '更新令牌失败'
    },
    stats: {
        title: '统计',
        daily: '每日',
        weekly: '每周',
        monthly: '每月',
        totalRequests: '总请求数',
        successful: '成功',
        errors: '错误',
        totalTokens: '总令牌数',
        endpointBreakdown: '端点明细',
        endpoint: '端点',
        requests: '请求数',
        inputTokens: '输入令牌',
        outputTokens: '输出令牌',
        noDataAvailable: '暂无数据',
        failedToLoad: '加载统计数据失败',
        apiKeyBreakdown: 'API Key 用量',
        apiKey: 'API Key',
        lastUsedAt: '最后使用'
    },
    testing: {
        title: '端点测试',
        selectEndpoint: '选择端点',
        runTest: '运行测试',
        loadingEndpoints: '加载中...',
        noEnabledEndpoints: '无启用的端点',
        pleaseSelectEndpoint: '请选择一个端点',
        testCompletedSuccessfully: '测试成功完成',
        testFailed: '测试失败',
        latency: '延迟',
        response: '响应',
        error: '错误',
        noResponse: '无响应',
        unknownError: '未知错误',
        failedToLoadEndpoints: '加载端点失败'
    },
    notifications: {
        endpointsReordered: '端点重新排序成功',
        endpointCreated: '端点创建成功',
        endpointUpdated: '端点更新成功',
        endpointDeleted: '端点删除成功',
        endpointCloned: '端点已克隆。请保存以创建新端点。',
        endpointSwitched: '已切换到端点：',
        endpointEnabled: '端点已启用',
        endpointDisabled: '端点已禁用',
        testSuccessful: '测试成功！延迟：',
        testFailed: '测试失败：',
        modelSelected: '已选择模型：',
        importDone: '导入完成：新增 {created}，更新 {updated}，跳过 {skipped}，失败 {failed}',
        credentialEnabled: '凭证已启用',
        credentialDisabled: '凭证已禁用',
        credentialActivated: '凭证已激活',
        tokenUpdated: '凭证令牌已更新',
        credentialDeleted: '凭证已删除',
        urlCopied: 'URL 已复制'
    },
    transformers: {
        claude: 'Claude',
        openai: 'OpenAI',
        openai2: 'OpenAI 响应',
        gemini: 'Gemini',
        deepseek: 'DeepSeek'
    },
    apikeys: {
        title: 'API 密钥',
        subtitle: '访问控制管理',
        addKey: '添加密钥',
        editKey: '编辑密钥',
        deleteKey: '删除密钥',
        keyName: '密钥名称',
        keyValue: '密钥值',
        copyKey: '复制密钥',
        regenerateKey: '重新生成',
        enabled: '启用',
        disabled: '禁用',
        expiresAt: '过期时间',
        never: '永不过期',
        lastUsed: '最后使用',
        endpoints: '可访问端点',
        selectEndpoints: '选择端点',
        noKeys: '暂无 API 密钥',
        noKeysMessage: '创建 API 密钥以控制访问权限',
        confirmDelete: '确定要删除密钥"{name}"吗？',
        confirmRegenerate: '重新生成密钥将使旧密钥失效，确定继续吗？',
        keyCreated: '密钥创建成功',
        keyUpdated: '密钥更新成功',
        keyDeleted: '密钥删除成功',
        keyRegenerated: '密钥重新生成成功',
        keyCopied: '密钥已复制到剪贴板',
        authEnabled: '启用 API Key 认证',
        authDisabled: '禁用 API Key 认证',
        authEnabledHelp: '启用后，所有代理请求必须提供有效的 API Key',
        configUpdated: '配置已更新',
        failedToLoad: '加载失败',
        failedToCreate: '创建失败',
        failedToUpdate: '更新失败',
        failedToDelete: '删除失败',
        failedToRegenerate: '重新生成失败',
        failedToUpdateConfig: '更新配置失败'
    }
};
